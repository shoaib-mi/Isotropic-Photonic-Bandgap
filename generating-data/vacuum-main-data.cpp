// use this line to speed up running time -> g++ -I c:\c_libs -O3 -march=native -o vacuum-main-data vacuum-main-data.cpp
...
cd round(cd x, int decimal){
	double re = round(x.real()*pow(10, decimal)) / pow(10, decimal);
	double im = round(x.imag()*pow(10, decimal)) / pow(10, decimal);
	return cd(re, im);
}


// this function produce and save the data into output file
int produce_data(string fname, double omega, double theta, double phi){
	...
	if(output_file.is_open()){
		t[0] = t_min;
		c1[0] = sin(theta/2) * exp(cd(0.0, phi));
    	c2[0] = cd( cos(theta/2), 0.0);
...
int main(int argc, char* argv[]){
	...
	int result;
	result = produce_data(path + "ket1.csv", omega, theta, phi_p);
	result = produce_data(path + "ket2.csv", omega, theta, phi_p + phase_step);
	result = produce_data(path + "ket3.csv", omega, theta + phase_step, phi_p);
