// use this line to speed up running time -> g++ -I C:\c_libs -O3 -march=native -o hss-measurements2 hss-measurements2.cpp
...
Matrix3cd density_matrix(cd h1, cd h2, cd h0){
    ...
    cd rho_12 = h1 * conj(h2) * exp(-cd(0.0,omega_21*t));
    cd rho_13 = h1 * conj(h0) * exp(cd(0.0,delta_10*t));
    cd rho_23 = h2 * conj(h0) * exp(cd(0.0,delta_20*t));
