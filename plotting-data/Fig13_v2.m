...
i = 6; % Coherence is the sixth column of the data
j = 6; % Coherence is the sixth column of the data
x_lim = [0 20];
y_lim = [0 1];
x_ticks = [0 10 20];
y_ticks = [0 0.5 1];
...
lgd1.Position = [0.25 0.5 0.2479 0.3961];
lgd2 = legend(ax2, [p5, p6, p7, p8],{'$\Omega=0.1\beta$', '$\Omega=0.5\beta$', '$\Omega=3\beta$','$\Omega=5\beta$'}, FontSize=legend_fontsize*.9, Interpreter='latex', Box="off");
lgd2.ItemTokenSize = [20 6];
lgd2.Position = [0.74 0.6 0.2493 0.2961];%= [0.72 0.6 0.3 0.3];

%% marking figures
text(ax1, 0.82, 0.13, '(a)', 'Units','normalized','FontSize',marker_fontsize,'FontWeight',marker_fontweight,'FontName',marker_fontname)
text(ax2, 0.85, 0.13, '(b)', 'Units','normalized','FontSize',marker_fontsize,'FontWeight',marker_fontweight,'FontName',marker_fontname)
% 0.82
%% Save and export figurs
if ~exist('ghafar_plots', 'dir')
    mkdir('ghafar_plots');
end
fname = 'ghafar_plots\Fig13-v2';
