...
i = 6; % f_phi_phi is the second column of the data
j = 6; % f_theta_theta is the fourth column of the data
x_lim = [0 20];
y_lim = [0 1];
x_ticks = [0 10 20];
y_ticks = [0 0.5 1.0];
...
p4 = plot(ax1, region_1_data(:,1), region_1_data(:,j), '-','LineWidth', curve_linewidth,'color',colors{1});
...
%% add legend
lgd = legend([p4, p2, p3],{'$\omega_{3c}=0.9\beta$', '$\omega_{3c}=0.2\beta$', '$\omega_{3c}=-1\beta$'}, FontSize=legend_fontsize*.9, Interpreter='latex', Box='off');
lgd.ItemTokenSize = [20 6];
lgd.Position = [0.55 0.70 0.3532 0.2500];

%% Save and export figurs
if ~exist('ghafar_plots', 'dir')
    mkdir('ghafar_plots');
end
fname = 'ghafar_plots\Fig12';
