...
%%  This code plots F_phi and F_theta for phi in [0, pi/2, pi] vs. (gamma t) and (beta t) for free space and pbg
...
p2 = plot(ax1, vac_phi_pi_2_data(:,1), vac_phi_pi_2_data(:,i), '--', LineWidth=curve_linewidth, Color=colors{2});%, MarkerSize=1,MarkerIndices=1:10:length(vac_phi_pi_2_data(:,1)));
...
plot(ax3, vac_phi_0_data(:,1), vac_phi_0_data(:,j), '-','LineWidth', curve_linewidth,'Color',colors{1});
plot(ax3, vac_phi_pi_2_data(:,1), vac_phi_pi_2_data(:,j),'--','LineWidth', curve_linewidth,'color',colors{2});
plot(ax3, vac_phi_pi_data(:,1), vac_phi_pi_data(:,j), '-.','LineWidth', curve_linewidth,'color',colors{3});
...
plot(ax4, pbg_phi_0_data(:,1), pbg_phi_0_data(:,j), '-','LineWidth', curve_linewidth,'Color',colors{1});
plot(ax4, pbg_phi_pi_2_data(:,1), pbg_phi_pi_2_data(:,j),'--','LineWidth', curve_linewidth,'color',colors{2});
plot(ax4, pbg_phi_pi_data(:,1), pbg_phi_pi_data(:,j), '-.','LineWidth', curve_linewidth,'color',colors{3});

%% add legend
lgd = legend(ax1, [p1, p2, p3],{'$\phi=0$', '$\phi=\pi/2$', '$\phi=\pi$'}, Interpreter='latex', Box="off");%, FontSize=legend_fontsize
lgd.ItemTokenSize = [20 6];
lgd.Position = [0.2654 0.80 0.2253 0.1387];
