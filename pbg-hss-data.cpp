// use this line to speed up running time -> g++ -I C:\c_libs -O3 -march=native -o hss-main hss-main.cpp
...
string to_string_array(string arr[], int size){
	string result = "";
...
string to_string_array(double arr[], int size){
	string result = "";
	for(int k=0;k<size;k++){
		result += to_string(arr[k]) + " ";
	} 
    return(result);
}

// this function produce and save the data into output file
int produce_data(string fname, int region, double omega, double phi){
	ofstream output_file(fname);

	// define variables
	double delta_10, delta_20;
	VectorXd t(length);
	...
	if(output_file.is_open()){
		t[0] = t_min;
		h1[0] = exp(i * phi) / sqrt(3);
    	h2[0] = 1.0 / sqrt(3) + 0.0*i;
...
int main(int argc, char* argv[]){
	...
	if (argc != 4){
		...
		cout << "omega: 0..9  <==>  " << to_string_array(omega_set, omega_set_size) << endl;
		cout << "phi = 0..3   <==>  " << to_string_array(phi_name_set, 4) << endl;
	...
	int result;
	result = produce_data(path + "ket1.csv", region, omega, phi);
	result = produce_data(path + "ket2.csv", region, omega, phi + phase_step);

	clock_t end = clock(); // end time
    double duration = double(end - start) / CLOCKS_PER_SEC;
    cout << "Producing data for ket1 & ket2 took " << duration << " seconds" << endl;
	return(0);
}